...
VV法が調和振動子の場合にエネルギーを保存するのは、時間発展を記述する行列の行列式が1であることに対応していた。一般の系の場合は時間発展を記述するのが演算子ではなく演算子になる。ここでは、VV法がエネルギーを保存するシンプレクティック積分であることを示すそう。

運動方程式が与えられればリュービル演算子の表式がわかる。そして、リュービル演算子がわかれば、それを指数の肩に乗せれば時間発展演算子が得られるのであった。この時間発展演算子の表式が厳密に求められれば、その系は「解けた」ことになる。その、厳密に解ける例として、調和振動子を考えよう。
...
と書ける。いま、リュービル演算子が行列$iL$で書けているので、これを指数の肩の上に乗せると時間発展行列(演算子)が得られる。
...
さて、今回はある行列$iL$を指数の肩に乗せた$\exp(ihL)$の表式が厳密に求められたが、そのためには$iL$を対角化する必要があり、もし対角化できたら問題は解けたと同義である。そこで、$\exp(ihL)$を近似することを考えよう。

いま、行列$iL$が、$iL = A + B$と二つの行列の和で書けるとしよう。

> 詳しい人向けの注：きちんとやるなら$A$や$B$にも虚数単位$i$をつけて$A$や$B$をエルミートにすべきだが、式が煩雑になるので以下では$A$や$B$には虚数単位をつけないことにする。そのため、$A,B$がエルミートではなく歪エルミートになっていることに注意。

TODO: 真面目に虚数単位つけたほうがいいかなぁ。
...
* ハミルトンの運動方程式に対応する時間微分行列(リュービル演算子)$L$はエルミート行列となる
* エルミート行列$L$に虚数単位をつけた$iL$は歪エルミート行列となる
* 時間発展行列$U=\mathrm{e}^{iL}$は、歪エルミート行列$iL$を指数関数の肩に乗せたものなので、ユニタリ行列になる
* ユニタリ行列の行列式は1となる。
* 行列式が1となる行列による変換は、面積要素を保存する。これによりエネルギーが保存する。
* 指数分解公式は、時間微分行列$iL$を冪零行列の和$A+B$で表し、時間発展演算子を冪零行列を指数の肩に乗せたもので表現する方法である。
* 冪零行列を指数の肩に乗せると行列式が1となるので、$|\exp(h A)|=|\exp(h B)|=1$である。以上からこの積で作られた行列の行列式が1となり、時間発展がシンプレクティックとなる

## 微分演算子による表現

調和振動子の場合は、リュービル演算子、時間発展演算子が行列で書けた。しかし、一般に時刻$t$の座標$(p,q)$から時刻$t+h$の座標$(P,Q)$への写像は非線形となり、それぞれの演算子が行列では書けない。この場合の指数分解公式と、シンプレクティック性について見てみよう。

ハミルトンの運動方程式におけるリュービル演算子は以下のように書けるのであった。
...
## Velocity Velret法がシンプレクティックである証明

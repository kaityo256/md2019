...
この系が時刻$t$で$(p,q)$にいて、時間発展の結果、時刻$t+h$に$(P,Q)$に移動したとする。また、この系の物理量$A(p(t),q(t))$が定義されているとしよう。$A$は$p,q$のみの関数であり、以下$A(p(t),q(t))$を$A(t)$と略記する。

さて、系が時間発展するにつれて、物理量$A$も時間変化する。$A$を用いて、この系の時間発展演算子$U(h)$を、以下のように定義する。
